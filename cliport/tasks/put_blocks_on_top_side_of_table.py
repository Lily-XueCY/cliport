...
class PutBlockonTopSide(Task):
    """Put-blocks-on-top-side-of-table"""

    def __init__(self):
        super().__init__()
        self.max_steps = 12
        self.lang_template = "put the {pick} block on the top side of the table"
...
#         env.add_object(base_urdf, base_pose, 'fixed')
        ...
        for i in range(6):
            ...
            targ = ((0.75-0.02, block_pose[0][1], block_pose[0][2]), block_pose[1])
            print(block_pose)
            print(targ)
            targs.append(targ)
