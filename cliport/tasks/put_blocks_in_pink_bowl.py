...
class PutBlockInPinkBowl(Task):
    ...
    def __init__(self):
        ...
        self.max_steps = 10
        self.pos_eps = 0.05
        self.lang_template = "put the {pick} block on a {place} block"
    ...
    def reset(self, env):
        ...
        print(n_bowls, n_blocks)

        all_color_names = self.get_colors()
        selected_color_names = random.sample(all_color_names, n_bowls+n_blocks-1)
        selected_color_names.append('pink')
        ...
        #Add pink bowl
        pink_bowl_pose = self.get_random_pose(env, bowl_size)
        bowl_id = env.add_object(bowl_urdf, pink_bowl_pose, 'fixed')
        p.changeVisualShape(bowl_id, -1, rgbaColor=utils.COLORS['pink'] + [1])
        bowl_poses.append(pink_bowl_pose)
        ...
        for i in range(n_blocks):
            ...
            p.changeVisualShape(block_id, -1, rgbaColor=colors[i+n_bowls-1] + [1])
            blocks.append((block_id, (0, None)))

        # Goal: put each block in a pink bowl.
...
#                                                          place=selected_color_names[1]))
        self.goals.append(([blocks[0]], np.ones((1, 1)), [pink_bowl_pose], False, True, 'pose', None, 1/len(blocks)))
        self.lang_goals.append("put the {pick} block in a pink bowl".format(pick=selected_color_names[n_bowls-1]))
        for i in range(1, n_blocks):
            self.goals.append(([blocks[i]], np.ones((1, 1)), [pink_bowl_pose], False, True, 'pose', None, 1/len(blocks)))
            self.lang_goals.append(self.lang_template.format(pick=selected_color_names[i+n_bowls-1],        place=selected_color_names[i+n_bowls]))
                           

        # Only one mistake allowed.
        self.max_steps = len(blocks) + 3
